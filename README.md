<h1 align="center">Hi 👋, I'm Roshan A Rauof</h1>
<h3 align="center">AI/ML Engineer — Voice AI & Indic Language Systems</h3>

---

### 🚀 About Me
- 🎓 B.Tech CSE (AI & ML), VIT Chennai — degree certificate arriving Aug/Sept 2026
- 🎙️ Focused on voice AI, ASR, and Indic-language NLP pipelines
- ⚙️ Currently at DRDO Bengaluru — C-based real-time systems, SIP/RTP, POSIX syscalls
- 🧵 Built ASR/multilingual pipelines (StoryWeaver) and anomaly detection systems (PIRS)

---

### 🛠️ Tech Stack
<p align="left">
  <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/LangChain-1C3C3C?style=for-the-badge&logo=langchain&logoColor=white" />
  <img src="https://img.shields.io/badge/FAISS-0467DF?style=for-the-badge&logo=meta&logoColor=white" />
  <img src="https://img.shields.io/badge/C-A8B9CC?style=for-the-badge&logo=c&logoColor=white" />
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />
</p>

*(Add Pipecat / Sarvam AI badges once you confirm shields.io logo support — some niche tools don't have icons, plain text badges work fine.)*

---

### 📌 Featured Projects
| Project | Description | Stack |
|---|---|---|
| 🗣️ **Govt Scheme Voice Assistant** | Multilingual (Hindi + Tamil) voice assistant for government scheme eligibility, with hallucination guardrails on low-confidence retrieval | Pipecat · Sarvam AI · LangChain · FAISS |
| 🧵 **StoryWeaver** | ASR and multilingual pipeline for Indic languages | Python · [fill in actual ASR stack] |
| 🕵️ **PIRS** | Insider threat / anomaly detection system | Python · TensorFlow · scikit-learn · SHAP |
| 🍳 **Kitchen Safety Verification Agent** | [one line once you finalize the stack] | [fill in] |

---

### 📊 GitHub Stats

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api?username=roshnrf&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" />
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=roshnrf&layout=compact&theme=tokyonight&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=roshnrf&theme=tokyonight&hide_border=true" />
</p>

---

### 📫 Connect
<p align="left">
  <a href="mailto:roshanabdlrf03@gmail.com"><img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://github.com/roshnrf"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" /></a>
</p>
